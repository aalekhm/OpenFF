...
public class ScreenEncoder implements Device.RotationListener
{
					...
					do
					{
						try
						{
							...
							switch(MAGIC)
							{
								...
								{
									LogD.LogConsole("Directory Listing");
									iFileCount = 0;
	...
	class Dir
	{
		...
		public Vector<Dir>		m_vChildren;
		public boolean			m_bIsDirectory;

		public Dir(String sName, String sPath)
		{
			m_sName = sName;
			m_sPath = sPath;
			m_vChildren = new Vector<Dir>();
			m_bIsDirectory = false;
		}

		private StringBuffer printTabs(int iTabCount)
		{
			StringBuffer sSpaces = new StringBuffer();
			int iSize = 3 * iTabCount;
		...
		public void list(int iTabCount)
		{
			StringBuffer sSpaces = new StringBuffer();
			sSpaces = printTabs(iTabCount);

			LogD.LogConsole(sSpaces.toString()+"[+] "+m_sName+" : "+m_vChildren.size());
			for(int i = 0; i < m_vChildren.size(); i++)
			{
				Dir pChild = m_vChildren.elementAt(i);

				if(pChild.m_bIsDirectory)
				{
					pChild.list(iTabCount + 1);
				}
				else
				{
					LogD.LogConsole(sSpaces.toString()+" - "+pChild.m_sName);
				}
			}
		}
	};
	...
	private void DirectoryList(Dir dir)
	{
		StringBuffer sSpaces = new StringBuffer();
		...
		String sParentPath = dir.m_sPath;
		//LogD.LogConsole(sSpaces.toString()+"[+] "+sParentPath);

		File f = new File(sParentPath);
		...
		if(files != null && files.length > 0)
		{
			...
			{
				String sChildName = inFile.getName();
				String sChildPath = sParentPath+"/"+sChildName;

				Dir pChild = new Dir(sChildName, sChildPath);
				dir.m_vChildren.add(pChild);

				if (inFile.isDirectory()) 
				{
					pChild.m_bIsDirectory = true;
					//DirectoryList(pChild);
				}
				else
				{
					iFileCount++;
					//LogD.LogConsole(sSpaces.toString()+"- "+inFile.getName());
				}
			}
