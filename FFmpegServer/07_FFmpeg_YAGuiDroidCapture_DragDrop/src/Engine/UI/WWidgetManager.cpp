...
#define REGISTER_WIDGET(__widgetName__) WComponentFactory::Get()->Register(#__widgetName__, &__widgetName__::Create);
...
void WWidgetManager::init(int32_t iScreenWidth, int32_t iScreenHeight)
{
	...
	REGISTER_WIDGET(WContainer)
	REGISTER_WIDGET(WStatic)
	REGISTER_WIDGET(WButton)
	REGISTER_WIDGET(WCheckbox)
	REGISTER_WIDGET(WTextField)
	REGISTER_WIDGET(WScrollbar)
	REGISTER_WIDGET(WTextBox)
	REGISTER_WIDGET(WListBox)
	REGISTER_WIDGET(WComboBox)
	REGISTER_WIDGET(WConsoleLog)
	REGISTER_WIDGET(WWindow)
	REGISTER_WIDGET(WTabbedPane)
	REGISTER_WIDGET(WFrame)
	REGISTER_WIDGET(WTree)
	REGISTER_WIDGET(WTable)
	REGISTER_WIDGET(WDummy)
	REGISTER_WIDGET(WCanvas)
	REGISTER_WIDGET(WInspectorTab)
	REGISTER_WIDGET(WInspector)
	REGISTER_WIDGET(WGraph)
	REGISTER_WIDGET(WSprite)
	...
	ADD_UI_WINDOW(NULL, m_pBaseWindow, "Title", 0, 0, 0, m_iScreenWidth, m_iScreenHeight, ROOT_WINDOW_ID, ID_TYPE_WND_C)
	((WWindow*)m_pBaseWindow)->setVisible(true);
	
	initOpenGLBuffers();
	if (initShaders(m_iProgramID))
	{
		initOrthogonalMatrix(m_iScreenWidth, m_iScreenHeight);
	}
}
...
void WWidgetManager::update(uint32_t iDeltaTimeMs, uint64_t lElapsedTime)
{
	if(m_pBaseWindow) 
	{
		///////////////////////////////////////
		...
		///////////////////////////////////////
		((WContainer*)m_pBaseWindow)->frameUpdate(iDeltaTimeMs);
		WWidgetManager::onEvent((H_WND)this, WN__UPDATE, -1, lElapsedTime);
...
void WWidgetManager::paint()
{
	glEnable(GL_BLEND);
	glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);

	((WContainer*)m_pBaseWindow)->frameRender();
	WWidgetManager::onEvent((H_WND)this, WN__PAINT, -1, -1);
...
void WWidgetManager::initOpenGLBuffers()
{
...
void WWidgetManager::initOrthogonalMatrix(int32_t iScreenWidth, int32_t iScreenHeight)
{
	float fOrtho[16];
	{
		float oneOverHalfWidth = 1.0f / (iScreenWidth >> 1);
		float oneOverHalfHeight = 1.0f / (iScreenHeight >> 1);
	...
	glUseProgram(m_iProgramID);
	{
		m_Mat2DOrthogonalTransform = glm::ortho(0.0f, (float)iScreenWidth, (float)iScreenHeight, 0.0f);
...
void WWidgetManager::resizeWindow(int32_t iNewWidth, int32_t iNewHeight)
{
	m_iScreenWidth = iNewWidth;
	m_iScreenHeight = iNewHeight;

	DWORD dSize = ((m_iScreenHeight & 0xFFFF) << 16) | ((m_iScreenWidth) & 0xFFFF);
	SendMessageQ(m_pBaseWindow, WM__SETSIZE, NULL, dSize);

	initOrthogonalMatrix(iNewWidth, iNewHeight);
}
...
H_WND WWidgetManager::addComponentFromWidget(	std::string sComponentClass, 
												std::string sParentWidget, 
												std::string sChildWidget, 
												RectF wndRect,
												H_WND hParentWnd,
												HMENU hMenu,
												LPVOID lpVoid,
												std::function<void(CHILD* pChildWidget, RectF destRect, RectF& childRect)> fSetChildRectCallable)
{
	RectF destRect, childRect;
	H_WND hWnd = NULL;
	WIDGET* pParentWidget = WWidgetManager::getWidget(sParentWidget.c_str());
	if (pParentWidget != nullptr)
	{
		CHILD* pChildWidget = pParentWidget->getChild(sChildWidget.c_str());

		destRect = getDestinationRectFromWidgetForChild(sParentWidget, sChildWidget, wndRect);
		fSetChildRectCallable(pChildWidget, destRect, childRect);
		hWnd = 
		CreateComponent(	sComponentClass.c_str(), 
							"", 
							0, 
							childRect.X,
							childRect.Y,
...
RectF WWidgetManager::getDestinationRectFromWidgetForChild(std::string sParentWidget, std::string sChildWidget, RectF wndRect)
{
	RectF destRect;
	WIDGET* pParentWidget = WWidgetManager::getWidget(sParentWidget.c_str());
	if (pParentWidget != nullptr)
	{
		RectF idealRect;
		CHILD* pChildWidget = pParentWidget->getChild(sChildWidget.c_str());
		idealRect = pChildWidget->posOffsets;
		WWidgetManager::getDestinationRect(	destRect,
											pParentWidget->widgetSize.width,
											pParentWidget->widgetSize.height,
											&wndRect,
											&idealRect,
											pChildWidget->align.eHAlign,
											pChildWidget->align.eVAlign
										);
	}
...
RectF WWidgetManager::getDestinationClientRectFromWidget(std::string sParentWidget, RectF wndRect)
{
	RectF destRect;
	WIDGET* pParentWidget = WWidgetManager::getWidget(sParentWidget.c_str());
	if (pParentWidget != nullptr)
	{
		RectF idealRect;
		CLIENT_AREA* clientArea = pParentWidget->getClientAreaAt(0);
		idealRect = clientArea->posOffsets;
		WWidgetManager::getDestinationRect(	destRect,
											pParentWidget->widgetSize.width,
											pParentWidget->widgetSize.height,
											&wndRect,
...
void WWidgetManager::getClientRectFromWidget(RectF& destRect, std::string sParentWidget, RectF wndRect)
{
	WIDGET* pParentWidget = WWidgetManager::getWidget(sParentWidget.c_str());
	if (pParentWidget != nullptr)
	{
		RectF idealRect;
		CLIENT_AREA* pClientArea = pParentWidget->getClientAreaAt(0);
		idealRect = pClientArea->posOffsets;
		WWidgetManager::getDestinationRect(	destRect,
											pParentWidget->widgetSize.width,
											pParentWidget->widgetSize.height,
											&wndRect,
											&idealRect,
											pClientArea->align.eHAlign,
											pClientArea->align.eVAlign
										);
	}
}
...
H_WND WWidgetManager::GetWindowQ(H_WND hWnd, UINT ID_WINDOW)
{
	if (ID_WINDOW >= 0 && hWnd != NULL)
	{
		WContainer* pComp = (WContainer*)hWnd;
		return ((WContainer*)pComp)->GetWindow(ID_WINDOW);
	}
