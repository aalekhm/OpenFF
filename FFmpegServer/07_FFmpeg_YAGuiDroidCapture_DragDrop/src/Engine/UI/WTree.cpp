...
void WTree::onCreateEx(LPVOID lpVoid) 
{
	...
	destRect = WWidgetManager::getDestinationRectFromWidgetForChild("Tree", "VScroll", wndRect);
	{
	...
	destRect = WWidgetManager::getDestinationRectFromWidgetForChild("Tree", "HScroll", wndRect);
	{
...
void WTree::updateVBarCursorPosition()
{
	float _part = abs(m_iMainY);
	float _total = m_iMaxHeightPixels;
	float _percentage = (_part / _total) * 100;
...
void WTree::updateHBarCursorPosition()
{
	float _part = abs(m_iMainX);
	float _total = m_iMaxWidthPixels;
	float _percentage = (_part / _total) * 100;
...
void WTree::renderTree() 
{
	...
	for(int i = iStartNode, iLeafYCount = iStartNode; i < m_vLeafList.size(); i++, iLeafYCount++) 
	{
		...
		else 
		{
			...
			pRenderer->drawStringFont(it->m_sName.c_str(), leafX + LEAF_SPACING_X + LEAF_NAME_TEXT_X, ileafY, 0);
		}
	...
	if(!true) 
	{
		...
		str << "sn=" << iStartNode << "/" << (iStartNode + maxLinesPerPage) << ", " << m_vLeafList.size();

		pRenderer->drawStringFont(str.str().c_str(), thisWndRect.X+5, thisWndRect.Y+5, 0);
	}
}
...
void WTree::addToParent(TREEITEM* pChild, TREEITEM* pParent)
{
	if (pChild != nullptr)
	{
		if (pParent == nullptr)
		{
			pChild->setParent(m_pRoot);
			m_pRoot->addLeaf(pChild);
		}
		else
		{
			pChild->setParent(pParent);
			pParent->addLeaf(pChild);
		}
	}
}

TREEITEM* WTree::getRoot()
{
	return m_pRoot;
}
...
void WTree::onResize(int width, int height)
{
...
LRESULT WTree::OnSendMessage(UINT msg, WPARAM wParam, LPARAM lParam) 
{
	switch(msg) 
	{
		...
		case WM__SETSIZE:
		{
			...
			onResize((dwSize & 0xffff), ((dwSize >> 16) & 0xffff));
		}
		break;
		case WM__RESIZE:
		{
