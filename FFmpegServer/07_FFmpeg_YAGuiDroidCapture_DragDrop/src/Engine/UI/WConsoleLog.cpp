...
void WConsoleLog::drawLineNumbers() 
{
	if(HAS_LINE_NO) 
	{
		...
		for(int i = 0; i <= endLineNo; i++) 
		{
			...
			dbStr = std::to_string(startLineNo+i);
			WWidgetManager::setColor(0, 0, 0, 1.0f);
			WWidgetManager::drawStringFont(dbStr.c_str(), getLeft() + TB_LINE_NO_SPACE - TB_LEFT_GUTTER, m_minY+(i*LINE_HEIGHT), 2);
			WWidgetManager::resetColor();
		}
	}
}
...
void WConsoleLog::setTBLineNoSpace() 
{
	...
	TB_LINE_NO_SPACE = WWidgetManager::getStringWidthTillPos(sLen.str().c_str(), strlen(sLen.str().c_str())) + (TB_LEFT_GUTTER << 1);
