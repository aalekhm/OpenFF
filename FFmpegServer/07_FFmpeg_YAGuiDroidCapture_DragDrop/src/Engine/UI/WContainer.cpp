...
{	
	...
	// Custom Render Event 'WN__PAINT'!
	customRender();

	postRenderInternal();
}

void WContainer::customRender()
{
	// Previous Clip Rect
	RectF reclaimRect;
	WWidgetManager::GetClipBounds(&reclaimRect);
	...
	// Component Rect
	RectF componentRect(getLeft(), getTop(), getWidth(), getHeight());

	// Intersection Rect of the above '2'!
	RectF clipRect;
	clipRect.Intersect(clipRect, reclaimRect, componentRect);

	// Actual Clip
	WWidgetManager::setClip(clipRect.X, clipRect.Y, clipRect.Width, clipRect.Height);
	{
		WWidgetManager::onEvent((H_WND)this, WN__PAINT, getComponentID(), (LPARAM)&reclaimRect);
	}
	WWidgetManager::setClip(reclaimRect.X, reclaimRect.Y, reclaimRect.Width, reclaimRect.Height);
}
...
void WContainer::onResize(DWORD dwSize)
{
...
void WContainer::doResizeChildrens(DWORD dwSize)
{
	for (int i = 0; i < m_pChildren.size(); i++)
	{
		WComponent* pWComponent = m_pChildren[i];
		SendMessageQ(	pWComponent,
						WM__SETSIZE,
						NULL,
						dwSize);
	}
...
LRESULT WContainer::OnSendMessage(UINT msg, WPARAM wParam, LPARAM lParam)
{
	switch (msg)
	{
		case WM__SETSIZE:
