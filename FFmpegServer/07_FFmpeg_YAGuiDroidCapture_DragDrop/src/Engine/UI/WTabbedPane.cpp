...
{	
	...
	{
		m_ButtonScrollLeft = (WButton*)WWidgetManager::addComponentFromWidget(	"WButton",
																		"Tab", 
																		"ButtonLeft", 
																		wndRect, 
																		this, 
	...
	destRect = WWidgetManager::getDestinationRectFromWidgetForChild("Tab", "ButtonLeft", wndRect);
	{
		m_ButtonScrollLeft->setPosition(destRect.X - m_iLeft, destRect.Y - m_iTop);
		m_ButtonScrollLeft->setSize(destRect.Width, destRect.Height);
	...
	{
		m_ButtonScrollRight = (WButton*)WWidgetManager::addComponentFromWidget(	"WButton",
																		"Tab", 
																		"ButtonRight", 
																		wndRect, 
																		this, 
	...
	destRect = WWidgetManager::getDestinationRectFromWidgetForChild("Tab", "ButtonRight", wndRect);
	{
		m_ButtonScrollRight->setPosition(destRect.X - m_iLeft, destRect.Y - m_iTop);
		m_ButtonScrollRight->setSize(destRect.Width, destRect.Height);
		m_ButtonScrollRight->setPostRender(true);
	}
	///////////////////////////////////////////////////
	updateClientRect("Tab", wndRect);
	///////////////////////////////////////////////////
...
void WTabbedPane::onRender() 
{
	...
	for(int i = m_StartTabId; i < m_TabCount; i++, count++) 
	{
		...
		renderer->renderWidget((i == m_SelectedTab) ? "TabPatchSelected" : "TabPatchUnSelected", &rTab);
...
void WTabbedPane::resizeTab(WFrame* wTab)
{
	if (wTab != nullptr)
	{
		wTab->setPosition(m_ClientRect.X, m_ClientRect.Y);
		wTab->setSize(m_ClientRect.Width, m_ClientRect.Height);
	}
}
...
void WTabbedPane::onResize()
{
	onCreateEx((LPVOID)0);
}

void WTabbedPane::doResizeChildrens(DWORD dwSize)
{
	WFrame* wTab = nullptr;
	for (int i = 0; i < m_TabCount; i++)
	{
...
LRESULT WTabbedPane::OnSendMessage(UINT msg, WPARAM wParam, LPARAM lParam) 
{
	switch(msg) 
	{
		...
		case WM__GETSIZE:
		{
			...
			*dwSize |= ((int32_t)tabWndRect.Width & 0xffff);
			*dwSize |= (((int32_t)tabWndRect.Height & 0xffff) << 16);
	...
	WComponent::OnSendMessage(msg, wParam, lParam);
}
#endif