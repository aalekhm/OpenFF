...
void WComboBox::onCreateEx(LPVOID lpVoid) 
{
	...
	// TextField
	{
		...
		if (m_cbTextField == nullptr)
		{
			m_cbTextField = (WTextField*)WWidgetManager::addComponentFromWidget(	"WTextField",
																					"ComboBox", 
																					"TextField", 
																					wndRect, 
																					this, 
																					...
																					{
																						childRect.X = pChildWidget->posOffsets.x;
																						childRect.Y = pChildWidget->posOffsets.y;
																						childRect.Width = destRect.Width;
																						childRect.Height = destRect.Height;
																					});
		}
		destRect = WWidgetManager::getDestinationRectFromWidgetForChild("ComboBox", "TextField", wndRect);
		m_cbTextField->setPosition(cbTextField->posOffsets.x, cbTextField->posOffsets.y);
		m_cbTextField->setSize(destRect.Width, destRect.Height);
	...
	// Drop-Down Button
	{
		if (m_cbButton == nullptr)
		{
			m_cbButton = (WButton*)WWidgetManager::addComponentFromWidget(	"WButton",
																			"ComboBox", 
																			"ButtonDown", 
																			wndRect, 
																			this, 
		...
		destRect = WWidgetManager::getDestinationRectFromWidgetForChild("ComboBox", "ButtonDown", wndRect);
		m_cbButton->setPosition(destRect.X - m_iLeft, destRect.Y - m_iTop);
		m_cbButton->setSize(destRect.Width, destRect.Height);
	}

	// ListBox
	{
		CHILD* cbListBox = m_ComboBoxWidget->getChild("ListBox");
		if (m_cbListBox == nullptr)
		{
			m_cbListBox = (WListBox*)WWidgetManager::addComponentFromWidget(	"WListBox",
																				"ComboBox", 
																				"ListBox", 
																				wndRect, 
																				this, 
		...
		destRect = WWidgetManager::getDestinationRectFromWidgetForChild("ComboBox", "ListBox", wndRect);
		m_cbListBox->setPosition(destRect.X - m_iLeft, destRect.Y - m_iTop);
		m_cbListBox->setSize(destRect.Width, destRect.Height);
...
void WComboBox::addDefaultTestItems() 
{
	...
	ss.clear();
	ss << "c:\_projects\myyagui_20_7_combobox_memorymanaged. warning C4715: 'WListBox::getSize' : not all control paths return a value.";
	addItem(ss.str().c_str());
	ss << "Finished generating code";
	addItem(ss.str().c_str());
	ss << "Embedding manifest...";
	addItem(ss.str().c_str());
	ss << "Generating code";
	addItem(ss.str().c_str());
	ss << "MyYAGui - 0 error(s), 51 warning(s)";
	addItem(ss.str().c_str());
	ss << "Linking...";
	addItem(ss.str().c_str());
	ss << "Compiling...";
