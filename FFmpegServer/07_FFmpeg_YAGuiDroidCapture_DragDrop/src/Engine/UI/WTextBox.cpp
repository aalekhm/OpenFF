...
void WTextBox::onCreateEx(LPVOID lpVoid) 
{
	...
	destRect = WWidgetManager::getDestinationRectFromWidgetForChild("TextBox", "VScroll", wndRect);
	{
	...
	destRect = WWidgetManager::getDestinationRectFromWidgetForChild("TextBox", "HScroll", wndRect);
	{
...
void WTextBox::onRender() 
{
	...
	if(mState == READONLY) 
	{
		...
		WWidgetManager::getInstance()->fillRect(0.5f, 0.5f, 0.5f, 0.5f, &readOnlyRect);
	}
...
void WTextBox::drawLineNumbers() 
{
	if(HAS_LINE_NO) 
	{
		...
		for(int i = 0; i <= endLineNo; i++) 
		{
			...
			WWidgetManager::setColor(0, 0, 0, 1.0f);
			WWidgetManager::drawStringFont(dbStr.str().c_str(), getLeft() + TB_LINE_NO_SPACE - TB_LEFT_GUTTER, m_minY+(i*LINE_HEIGHT), 2);
			WWidgetManager::resetColor();
		}
	}
}
...
void WTextBox::setTBLineNoSpace() 
{
	...
	sLen << m_Lines.size();
	TB_LINE_NO_SPACE = WWidgetManager::getStringWidthTillPos(sLen.str().c_str(), sLen.str().size()) + (TB_LEFT_GUTTER << 1);
