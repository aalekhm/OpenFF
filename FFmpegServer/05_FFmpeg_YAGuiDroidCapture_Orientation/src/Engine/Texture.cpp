...
Texture* Texture::createEx(const char* path, bool generateMipmaps) {
	...
	Texture* texture = createTexture(path, generateMipmaps);
...
Texture* Texture::createTexture(const char* path, bool generateMipmaps)
{
	int iWidth, iHeight, iNRChannels;
	Texture* pTexture = NULL;

	stbi_set_flip_vertically_on_load(true);
	unsigned char *data = stbi_load(path, &iWidth, &iHeight, &iNRChannels, 0);
	if (data)
	{
		GLuint textureID = 0;
		...
		Texture::Format eFormat = Texture::UNKNOWN;
		if (iNRChannels == 3)		eFormat = Texture::RGB;
		else if (iNRChannels == 4)	eFormat = Texture::RGBA;

		glTexImage2D(GL_TEXTURE_2D, 0, (GLuint)eFormat, iWidth, iHeight, 0, (GLuint)eFormat, GL_UNSIGNED_BYTE, data);

		pTexture = new Texture();
		pTexture->m_hTexture = textureID;
		pTexture->m_Format = eFormat;
		pTexture->m_iWidth = iWidth;
		pTexture->m_iHeight = iHeight;
		pTexture->m_eType = TEXTURE_2D;
		if (generateMipmaps) 
		{
...
void Texture::saveToDisk(std::string sPath, STBImgType::Type eSTBImgType, int32_t iWidth, int32_t iHeight, STBImgFormat eSTBImgFormat, void* pRawData)
{
	switch (eSTBImgType)
	{
		case STBImgType::Type::BMP:
			stbi_write_bmp(sPath.c_str(), iWidth, iHeight, eSTBImgFormat, (uint8_t*)pRawData);
		break;
		case STBImgType::Type::TGA:
			stbi_write_tga(sPath.c_str(), iWidth, iHeight, eSTBImgFormat, (uint8_t*)pRawData);
		break;
		case STBImgType::Type::PNG:
			stbi_write_png(sPath.c_str(), iWidth, iHeight, eSTBImgFormat, (uint8_t*)pRawData, iWidth * (int32_t)eSTBImgFormat);
		break;
		case STBImgType::Type::JPG:
			stbi_write_jpg(sPath.c_str(), iWidth, iHeight, eSTBImgFormat, (uint8_t*)pRawData, 95);
		break;
		case STBImgType::Type::HDR:
			stbi_write_hdr(sPath.c_str(), iWidth, iHeight, eSTBImgFormat, (float*)pRawData);
		break;
	}
