	...
	public:
		...
		void resizeYAGui(int32_t iNewWidth, int32_t iNewHeight)
		{
			WWidgetManager* pWWidgetManager = WWidgetManager::getInstance();
			{
				...
				{
					H_WND hRecordWindow = pWWidgetManager->GetWindowQ(ID_WINDOW_RECORD);
					if (hRecordWindow != NULL)
					{
						DWORD dSize = (((iNewHeight - DROID_UI_WND_GUTTER_2H) & 0xFFFF) << 16) | ((iNewWidth - DROID_UI_WND_GUTTER_2W) & 0xFFFF);
						SendMessageQ(	hRecordWindow,
										WM__SETSIZE,
										NULL,
										dSize);
					}
			...
			switch (msg)
			{
				...
				case WM_BTN_LBUTTONUP:
				{
					switch (wParam)
					{
						...
						{
							//std::stringstream sAdbCommand;
							//sAdbCommand << RESOURCES << "adb shell am start -a android.intent.action.CALL -d tel:+919160902834";
							//adbExecute(sAdbCommand.str());
		...
		void addSplashUI(H_WND hRootWnd)
		{
			H_WND hWnd, hSplashFrame;
			int32_t iXStartPos = 0;
			int32_t iXPos = iXStartPos;
			int32_t iYPos = 0;
			ADD_UI_FRAME(	hRootWnd, 
							hSplashFrame, 
							"Droid Splash", 
							...
							ID_WINDOW_RECORD, 
							ID_TYPE_WND_C)
			((WFrame*)hSplashFrame)->setVisible(false);
		}
		...
		void addRecordUI(H_WND hRootWnd)
		{
			...
			ADD_UI_FRAME(hRootWnd, hRecordFrame, "Record Options", WM_ALIGN_WRT_PARENT_TOPCENTER | WM_ANCHOR_TOPCENTER, iXPos, iYPos, m_iWidth - DROID_UI_WND_GUTTER_2W, m_iHeight - DROID_UI_WND_GUTTER_2H, ID_WINDOW_RECORD, ID_TYPE_WND_C)
			((WFrame*)hRecordFrame)->setVisible(false);
			{
				// Recording Options
				{
					iXPos = iXStartPos;
					ADD_UI_BUTTON(hRecordFrame, hWnd, "Record", WM_ALIGN_WRT_PARENT_TOPLEFT | WM_ANCHOR_CENTERLEFT, iXPos + 20, iYPos, DROID_UI_BUTTON_W, DROID_UI_COMPONENT_H, ID_BUTTON_RECORD)
					SendMessageQ(hWnd, BM__ENABLE, NULL, NULL);
				
					iXPos += DROID_UI_BUTTON_W;
					ADD_UI_BUTTON(hRecordFrame, hWnd, "Stop", WM_ALIGN_WRT_PARENT_TOPLEFT | WM_ANCHOR_CENTERLEFT, iXPos + 20, iYPos, DROID_UI_BUTTON_W, DROID_UI_COMPONENT_H, ID_BUTTON_STOP)
					SendMessageQ(hWnd, BM__DISABLE, NULL, NULL);

					iYPos += iRowHeight;
					ADD_UI_LABEL(hRecordFrame, hWnd, "Filename :", WM_ALIGN_WRT_PARENT_TOPLEFT | WM_ANCHOR_CENTERLEFT, iXStartPos, iYPos, 75, DROID_UI_COMPONENT_H, ID_LABEL_FILENAME)
					ADD_UI_TEXTFIELD(hRecordFrame, hWnd, "Recorded", WM_ALIGN_WRT_PARENT_TOPLEFT | WM_ANCHOR_CENTERLEFT, iXStartPos + 75, iYPos, iTextField_Width, DROID_UI_COMPONENT_H, ID_TEXTFIELD_FILENAME)
					
					iYPos += iRowHeight;
					ADD_UI_LABEL(hRecordFrame, hWnd, "Directory :", WM_ALIGN_WRT_PARENT_TOPLEFT | WM_ANCHOR_CENTERLEFT, iXStartPos, iYPos, 75, DROID_UI_COMPONENT_H, ID_LABEL_DIRECTORY)
					ADD_UI_TEXTFIELD(hRecordFrame, hWnd, "..\\res", WM_ALIGN_WRT_PARENT_TOPLEFT | WM_ANCHOR_CENTERLEFT, iXStartPos + 75, iYPos, iTextField_Width, DROID_UI_COMPONENT_H, ID_TEXTFILED_DIRECTORY)
				}
				...
				// Rotation Options
				{
					iXPos = iXStartPos;
					iYPos += iRowHeight;
					ADD_UI_BUTTON(hRecordFrame, hWnd, "Freeze Rot", WM_ALIGN_WRT_PARENT_TOPLEFT | WM_ANCHOR_CENTERLEFT, iXPos + 20, iYPos, DROID_UI_BUTTON_W, DROID_UI_COMPONENT_H, ID_BUTTON_FREEZE_ROTATION)
					SendMessageQ(hWnd, BM__ENABLE, NULL, NULL);
					
					iXPos += DROID_UI_BUTTON_W;
					ADD_UI_BUTTON(hRecordFrame, hWnd, "Thaw Rot", WM_ALIGN_WRT_PARENT_TOPLEFT | WM_ANCHOR_CENTERLEFT, iXPos + 20, iYPos, DROID_UI_BUTTON_W, DROID_UI_COMPONENT_H, ID_BUTTON_THAW_ROTATION)
					SendMessageQ(hWnd, BM__ENABLE, NULL, NULL);
				}
				...
				{
					iYPos += iRowHeight;
					ADD_UI_LABEL(hRecordFrame, hWnd, "Screenshot Image Type(Press 'Ctrl-P/p' on main window):", WM_ALIGN_WRT_PARENT_TOPLEFT | WM_ANCHOR_CENTERLEFT, iXStartPos, iYPos, 75, DROID_UI_COMPONENT_H, ID_LABEL_IMAGE_EXPORT_TYPE)
					iYPos += (iRowHeight >> 1);
					ADD_UI_COMBOBOX(hRecordFrame, hWnd, "", 0, iXStartPos, iYPos, iTextField_Width, DROID_UI_COMPONENT_H * (int32_t)STBImgType::Type::MAX, ID_CB_IMAGE_EXPORT_TYPE)
					{
				...
				// QR Code - IP Address
				{
					...
					CreateComponentEx(	"WCanvas", 
										"WCanvas window", 
										WM_ALIGN_WRT_PARENT_TOPCENTER | WM_ANCHOR_CENTER, 
										iXStartPos,
										iYPos, 
										250, 
										250,
										hRecordFrame, 
										HMENU(ID_CANVAS_QRCODE),
										NULL);
				}
			}
		}
