...
std::unique_ptr<Sprite> Sprite::create(uint32_t iWidth, uint32_t iHeight)
{
	...
	if (pSprite != nullptr)
	{
		...
		{
			std::free(pSprite->m_pColourData);
			pSprite->m_pColourData = nullptr;
		}

		pSprite->m_iWidth = iWidth;
		pSprite->m_iHeight = iHeight;

		pSprite->m_pColourData = (Pixel*)std::malloc(iWidth * iHeight * sizeof(Pixel));
			...
			if (pImage != nullptr)
			{
				...
				switch (pImage->getFormat())
				{
					case Image::RGB:
					{
						...
						int32_t iSize = pSprite->m_iWidth * pSprite->m_iHeight;
...
{
	static uint32_t iSize = m_iWidth * m_iHeight;
	static uint32_t iPixel = p.getPixel();
	for (int32_t i = 0; i < iSize; i++)
	{
