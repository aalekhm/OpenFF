...
#define _INIT_FRAMEBUFFER_(__WIDTH__, __HEIGHT__) \
m_iWidth = iWidth; \
m_iHeight = iHeight; \
\
...
#define _DEINIT_FRAMEBUFFER_ \
glDeleteTextures(1, &m_glFrameBufferID); \
m_glFrameBufferID = 0; \
...
void Graphics::init(uint32_t iWidth, uint32_t iHeight)
{
	_DEINIT_FRAMEBUFFER_
	_INIT_FRAMEBUFFER_(iWidth, iHeight)
}
...
void Graphics::clear(uint8_t* pData)
{
	m_pFrameBuffer->clear((Pixel*)pData);
}
