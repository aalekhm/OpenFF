...
bool WWidgetManager::initShaders(GLint& iProgramID)
{
	...
	iSuccess = compileShaderFromFile(GL_VERTEX_SHADER, CORE_SPRITE_VERT_SHADER, iVertexShaderID);
	if (iSuccess >= 0)
	{
		iSuccess = compileShaderFromFile(GL_FRAGMENT_SHADER, CORE_SPRITE_FRAG_SHADER, iFragmentShaderID);
		if (iSuccess >= 0)
		{
...
GLint WWidgetManager::compileShaderFromFile(GLenum iShaderType, const char* sShaderFile, GLuint& iShaderID)
{
...
GLint WWidgetManager::compileShader(GLenum iShaderType, const char* sShaderSource, GLuint& iShaderID)
{
	GLint iSuccess = -1;
