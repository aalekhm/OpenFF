...
bool FFmpeg::loadFrame(const char* sFileName, uint32_t& iWidthOut, uint32_t& iHeightOut, uint8_t** pFrameDataOut)
{
	...
	uint8_t* pData = (uint8_t*)std::malloc(pAVFrame->width * pAVFrame->height * 4);

	// The 'Software Scaler'.
	SwsContext* pSwsContext = sws_getContext(	pAVFrame->width,
												pAVFrame->height,
	...
	uint8_t* pDest[4] = { pData, NULL, NULL, NULL };
